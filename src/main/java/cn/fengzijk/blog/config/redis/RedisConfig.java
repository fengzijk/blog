...
public class RedisConfig {
     ...
     */
    @Bean
    RedissonClient redissonSingle(@Value("${spring.redis.host}") String host, @Value("${spring.redis.password}") String password,
      @Value("${spring.redis.port}") int port) {
        Config config = new Config();
        SingleServerConfig serverConfig = config.useSingleServer().setAddress("redis://" + host + ":" + port).setTimeout(3000)
          .setConnectionPoolSize(32).setConnectionMinimumIdleSize(8);
